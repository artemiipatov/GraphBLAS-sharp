...
module internal Map2 =
        ...
        let preparePositions (op: Expr<'a option -> 'b option -> 'c option>) =
            <@
                fun
                    ...
                    (leftIndices: ClArray<int>)
                    (rightValues: ClArray<'b>)
                    (rightIndices: ClArray<int>)
    ...
    let run<'a, 'b, 'c when 'a: struct and 'b: struct and 'c: struct> op (clContext: ClContext) workGroupSize =
        ...
        let prepare = preparePositions<'a, 'b, 'c> op clContext workGroupSize
        ...
        fun
            (processor: MailboxProcessor<_>)
            allocationMode
            (leftVector: ClVector.Sparse<'a>)
            (rightVector: ClVector.Sparse<'b>) ->
    ...
    let assignByMask<'a, 'b when 'a: struct and 'b: struct> op (clContext: ClContext) workGroupSize =
        ...
        let prepare = preparePositionsAssignByMask op clContext workGroupSize
        ...
        let run<'a, 'b, 'c when 'a: struct and 'b: struct and 'c: struct> op (clContext: ClContext) workGroupSize =
            ...
            fun
                ...
                let allIndices, leftValues, rightValues, isLeft =
                    merge processor leftVector rightVector
