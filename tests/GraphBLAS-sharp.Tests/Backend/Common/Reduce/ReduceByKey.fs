...
let checkResult isEqual actualKeys actualValues keys values reduceOp =
    ...
    "Keys must be the same" |> Utils.compareArrays (=) actualKeys expectedKeys
...
let makeTest isEqual reduce reduceOp (arrayAndKeys: (int * 'a)[]) =
    let keys, values = Array.sortBy fst arrayAndKeys |> Array.unzip

    if keys.Length > 0 then
        let clKeys = context.CreateClArrayWithSpecificAllocationMode(DeviceOnly, keys)
...
let makeTest2D isEqual reduce reduceOp (array: (int * int * 'a)[]) =
    let firstKeys, secondKeys, values =
        array |> Array.sortBy (fun (fst, snd, _) -> fst, snd) |> Array.unzip3
    ...
    if firstKeys.Length > 0 then
        ...
        let resultLength =
            Array.length <| Array.distinctBy (fun (fst, snd, _) -> (fst, snd)) array
...
let checkResultOption isEqual keys values reduceOp actual =
    ...
    let expectedKeys, expectedValues =
        ...
        |> Array.map (fun (key, array) -> Array.map Some array |> Array.reduce reduceOp |> (fun result -> key, result))
    ...
    | Some(actualValues, actualKeys) ->
        "First keys must be the same" |> Utils.compareArrays (=) actualKeys expectedKeys
    ...
    if array.Length > 0 then
        ...
        |> Option.bind
            (fun ((clReducedValues, clFirstActualKeys, clSecondActualKeys): ClArray<'a> * ClArray<int> * ClArray<int>) ->
                let reducedFirstKeys = clFirstActualKeys.ToHostAndFree processor

                let reducedSecondKeys = clSecondActualKeys.ToHostAndFree processor
