...
let makeTest isEqual zero (array: 'a[,]) =
    let cooMatrix = Matrix.COO.FromArray2D(array, isEqual zero)

    let actual = cooMatrix.ToCSR

    let expected = Matrix.CSR.FromArray2D(array, isEqual zero)
...
let tests = [ createTest (=) 0; createTest (=) false ] |> testList "Convert"
